...
exports.wrap = function(x, min, max){
...
Vec2.prototype = {
	...
	rotateVec:function(v, center, angle){
		...
		var d = v.diff(v, center);
		d = d.rotate(d, angle);
		return center.add(center, d);
	},
	...
	ndot:function(a, b){
		...
		return (a.normalized(a).x * b.normalized(b).x) + (a.normalized(a).y * b.normalized(b).y);
	},
	cross:function(a, b){
		if(!(a instanceof Vec2)){throw "vec2.ndot: 1st argument not a Vec2() instance";return null;}
		if(!(b instanceof Vec2)){throw "vec2.ndot: 2nd argument not a Vec2() instance";return null;}
...
/****************************POLYGON*******************************/
PolN = function(position, vertices, orientation){
	this.pos = position;
